...
export default function MessageView({
  ...
  async function sendFeedback(rating: "HELPFUL" | "NOT_HELPFUL") {
    if (!messageId) return;
    try {
      await apiFetch("/chat/feedback", { method: "POST", token, body: { messageId, rating } });
      setFeedback(rating);
    } catch {
      setFeedback("error");
    }
      ...
      <div className="mt-4 flex flex-col gap-3">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <div className="flex h-6 w-6 items-center justify-center rounded-full bg-gradient-to-br from-pink-500 to-violet-500 text-[10px] font-bold text-white">Gr</div>
            <span className="text-xs font-medium text-slate-400">Knowledge Graph RAG</span>
          ...
          {messageId && feedback === null && (
            <div className="flex items-center gap-1">
              <button onClick={() => sendFeedback("HELPFUL")} className="rounded-md px-2 py-1 text-xs text-slate-400 hover:bg-white/5 hover:text-emerald-300" title="Helpful">
                👍
              </button>
              <button onClick={() => sendFeedback("NOT_HELPFUL")} className="rounded-md px-2 py-1 text-xs text-slate-400 hover:bg-white/5 hover:text-rose-300" title="Not helpful">
                👎
              </button>
            </div>
          )}
        ...
        {rels.length > 0 && (
          <div className="rounded-xl border border-white/10 bg-surface">
            <button
              ...
              className="flex w-full items-center justify-between gap-2 rounded-xl px-3 py-2.5 text-left hover:bg-white/5"
            >
            ...
            {sourcesOpen && (
              <div className="flex flex-wrap items-center gap-1.5 border-t border-white/10 px-3 py-3 text-xs">
                {rels.slice(0, 8).map((r, i) => (
                  <span key={r.rid ?? i} className="inline-flex items-center gap-1.5 rounded-lg bg-base-900/60 px-2 py-1 text-slate-300">
                    {r.source?.name}
            ...
            {sourcesOpen && (
              <div className="space-y-2 border-t border-white/10 p-3">
                {grouped.map((group) => {
                  ...
                  return (
                    <div key={group.documentId} className="overflow-hidden rounded-lg border border-white/10 bg-base-900/60">
                      ...
                      {isOpen && (
                        <div className="space-y-2 border-t border-white/10 p-2.5">
                          {group.items.map((c) => (
                            <div key={c.index} id={`cite-${c.index}`} className="rounded-md bg-base-950/50 p-2.5">
