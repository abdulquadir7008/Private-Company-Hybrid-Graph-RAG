...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-base bg-app-gradient">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-30 flex h-16 items-stretch gap-1 overflow-x-auto border-t border-rose-100 bg-white/95 px-2 backdrop-blur-xl md:hidden">
        ...
        <MobileNavItem href="/entities" label="Entities" icon={EntitiesIcon} active={isActive("/entities")} />
        <MobileNavItem href="/analytics" label="Analytics" icon={AnalyticsIcon} active={isActive("/analytics")} />
        <MobileNavItem href="/audit-logs" label="Audit Logs" icon={AuditLogsIcon} active={isActive("/audit-logs")} />
...
function MobileNavItem({ href, label, icon, active }: { href: string; label: string; icon: IconNode; active: boolean }) {
  return (
    <Link
      href={href}
      className={`flex min-w-[74px] flex-col items-center justify-center gap-1 rounded-lg text-[10px] font-medium transition ${
